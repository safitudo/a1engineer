...
# Runs INSIDE tmux. Always uses --print loop for Claude Code (works with both
# API key and session auth). This enables reliable nudging via the FIFO/inbox
# pipeline — interactive mode cannot be nudged because Claude Code's TUI
# doesn't process tmux send-keys Enter.
case "$AGENT_RUNTIME" in
  claude-code)
    touch /tmp/agent-mode-print  # signal to sidecar: use inbox path for nudges/IRC

...
function IrcMessage({ msg }) {
  ...
  return (
    <div className="flex gap-2 text-xs font-mono leading-relaxed hover:bg-[#161b22] px-2 py-0.5 rounded">
      <span className="text-[#8b949e] shrink-0 select-none">{time}</span>
      <span className="text-[#79c0ff] shrink-0">&lt;{msg.nick}&gt;</span>
      <span className="text-[#e6edf3] break-all">{msg.text}</span>
    </div>
  )
}
...
export default function TeamDetailPage() {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      setWsStatus('open')
      ws.send(JSON.stringify({ type: 'subscribe', teamId: id }))
    }
    ...
    ws.onmessage = (evt) => {
      ...
      if (msg.type === 'message') {
        ...
        setAgents(prev => prev.map(a =>
          a.id === msg.agentId ? { ...a, last_heartbeat: msg.timestamp } : a
        ))
      } else if (msg.type === 'agent_status') {
        // agent spawned/killed/stalled — re-fetch team to get updated agent list
        fetch(`/api/teams/${id}`)
          .then(r => r.ok ? r.json() : null)
          .then(data => { if (data) { setTeam(data); setAgents(data.agents ?? []) } })
          .catch(() => {})
      }
    }
  ...
  // ── Auto-scroll feed ────────────────────────────────────────────────────────
  useEffect(() => {
    if (!feedRef.current || !autoScrollRef.current) return
  ...
  const wsIndicator = {
    connecting: { cls: 'bg-[#d29922] animate-pulse', label: 'connecting' },
    open:       { cls: 'bg-[#3fb950] animate-pulse', label: 'live' },
    closed:     { cls: 'bg-[#8b949e]',               label: 'disconnected' },
    error:      { cls: 'bg-[#f85149]',               label: 'error' },
  }[wsStatus] ?? { cls: 'bg-[#8b949e]', label: wsStatus }
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] pt-14 flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden max-w-7xl mx-auto w-full px-6 py-6 gap-6">
        ...
        {/* Right: IRC feed */}
        <div className="flex-1 flex flex-col min-w-0 bg-[#161b22] border border-[#30363d] rounded-xl overflow-hidden">
          {/* Feed header */}
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#30363d] bg-[#0d1117]">
            <span className="text-xs font-mono text-[#8b949e]">IRC feed</span>
            <span className="inline-flex items-center gap-1.5 text-xs font-mono text-[#8b949e]">
              <span className={`w-1.5 h-1.5 rounded-full ${wsIndicator.cls}`} />
              {wsIndicator.label}
            </span>
          </div>
            ...
            {messages.length === 0 ? (
              <div className="flex items-center justify-center h-full">
                <p className="text-xs text-[#8b949e] font-mono italic">
                  {wsStatus === 'open' ? 'Waiting for messages…' : 'Connecting to IRC feed…'}
                </p>
              </div>
            ) : (
              messages.map((msg, i) => <IrcMessage key={i} msg={msg} />)
            )}
          </div>
        </div>
      </div>
